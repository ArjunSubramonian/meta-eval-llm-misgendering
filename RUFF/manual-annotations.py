...
ids = [559686475, 1680854028, 1619065260, 79186204, 52397784, 324082731, \
        1479686023, 1922373066, 1896232819, 701236152, 1613843142, 1624372326]
...
plt.tight_layout()
plt.savefig('plots/RUFF-pre-human-agreement.pdf')
...
plt.legend(bbox_to_anchor=(1.05, 1.05))
plt.tight_layout()
plt.savefig('plots/RUFF-post-human-agreement.pdf')
...
plt.legend()
plt.tight_layout()
plt.savefig('plots/RUFF-pre-extraneous.pdf')
...
plt.legend()
plt.tight_layout()
plt.savefig('plots/RUFF-post-extraneous.pdf')
...
sub_extr_samples_df = extr_samples_df[['\\textbf{Examples}']].map(lambda x: random.choice(x) if len(x) > 0 else '---')
